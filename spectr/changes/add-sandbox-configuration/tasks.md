...
## 1. Type Definitions
- [ ] 1.1 Add `SandboxIgnoreViolations` struct to `pkg/claude/options.go`
...
- [ ] 1.2 Add `SandboxNetworkConfig` struct to `pkg/claude/options.go`
...
- [ ] 1.3 Add `SandboxSettings` struct to `pkg/claude/options.go`
...
- [ ] 2.1 Add `Sandbox *SandboxSettings` field to the `Options` struct in `pkg/claude/options.go`
...
## 3. Documentation
- [ ] 3.1 Add godoc examples for sandbox configuration
...
- [ ] 4.2 Add unit tests for SandboxNetworkConfig JSON serialization
  - Test proxy port configurations
  - Test Unix socket allowlist serialization
  - Test AllowAllUnixSockets flag
- [ ] 4.3 Add unit tests for SandboxIgnoreViolations JSON serialization
  - Test file violation ignore list
  - Test network violation ignore list
  - Test combined file and network rules
- [ ] 4.4 Add integration test demonstrating sandbox configuration usage (optional)
  - Test that Options with Sandbox field passes to Claude CLI correctly
  - Verify JSON structure matches Python SDK format

## 5. Validation
- [ ] 5.1 Run `go build ./...` to ensure code compiles
- [ ] 5.2 Run `go test ./...` to ensure all tests pass
- [ ] 5.3 Run `golangci-lint run` to ensure linting passes
- [ ] 5.4 Verify JSON serialization matches Python SDK format expectations
- [ ] 5.5 Update tasks.md to mark all items as complete
